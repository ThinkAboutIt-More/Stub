...
function SwipeCard({ item, matchPct, matchConf, taste, collection, tmdb, onSkip, onWant, onRate, onTapInfo }) {
  ...
  return (
    ...
    >
      ...
      <div className="swipe-meta">
        ...
        <WhyWatch item={item} taste={taste} matchPct={matchPct} collection={collection} tmdb={tmdb} />
      </div>
  ...
  return (
    <div className="suggest-row" onClick={() => setExpanded((x) => !x)}>
      ...
      <div className="suggest-info">
        ...
        {taste && !item.aiReason && <WhyWatch item={item} taste={taste} matchPct={matchPct} collection={collection} tmdb={tmdb} />}
        {expanded && (
...
function buildWhyWatch(item, taste, matchPct, voteAvg, detail) {
  ...
  const hookBit = h1
    ? pick([`, built around ${h1}`, `, leaning on ${h1}`, h2 && h2 !== h1 ? `, heavy on ${h1} and ${h2}` : `, built around ${h1}`])
    : "";
  ...
  const va = voteAvg, vc = item.voteCount || 0;
  const reception = vc < 60 ? "still flying under the radar"
    : va >= 7.5 ? "broadly loved"
    : va >= 7 ? "strong crowd scores"
    : va >= 6 ? "decent crowd scores"
    : va >= 5 ? "mixed reception"
    : "rough crowd scores";
  const weights = getWeights(taste);
  ...
  let fit;
  if (pct >= 75) fit = pick([
    `${cap(reception)}, and it plays straight to your ${lane}taste.`,
    `${cap(reception)} - right in your ${lane}wheelhouse.`,
  ]);
  else if (pct >= 55) fit = pick([
    `${cap(reception)}; a comfortable fit for you.`,
    `${cap(reception)} and it lines up well with what you rate.`,
  ]);
  else if (pct >= 38) fit = pick([
    `${cap(reception)}, though it sits outside your usual lanes.`,
    `${cap(reception)} - the case here is the ${g1 ? g1.toLowerCase() : kind} itself, not your history.`,
  ]);
  else fit = pick([
    `${cap(reception)}; on your profile it's a stretch.`,
    `${cap(reception)}, but it's far from what you usually rate up.`,
  ]);
  return what + " " + fit;
}
...
function useFullDetail(tmdb, item) {
  ...
  useEffect(() => {
    let dead = false;
    if (!tmdb || !item) { setD(null); return; }
    const key = item.tmdbId + item.mediaType;
    if (key in fullDetailCache) { setD(fullDetailCache[key]); return; }
    tmdb.detailsFull(item.mediaType, item.tmdbId)
      .then((res) => { fullDetailCache[key] = res; if (!dead) setD(res); })
      .catch(() => { fullDetailCache[key] = null; });
    return () => { dead = true; };
  }, [tmdb, item && item.tmdbId, item && item.mediaType]);
  return d;
}

function WhyWatch({ item, taste, matchPct, collection, tmdb }) {
  ...
  const reason = useMemo(() => {
    ...
    if (matchPct != null && matchPct >= 70) {
      const enriched = detail ? { ...item, credits: item.credits || slimCredits(detail.credits) } : item;
...
export default function App() {
  ...
  function addToWatchlist(item) {
    setWatchlist((w) => {
      ...
      return [{ ...item, addedAt: Date.now() }, ...w];
    });
    fireBurst("want");
  }
