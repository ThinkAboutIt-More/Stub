...
function matchMetaFull(item, taste, people, crowd) {
  ...
  if (!unreleased && item.voteAverage != null && (item.voteCount ?? 0) >= 300) {
    ...
    if (pct > cap) { pct = cap; cappedBy = cap; }
  }
...
function explainMatch(item, taste, people, crowd, collection) {
  ...
  if (item.credits && people) {
    ...
    if (best) {
      ...
      (collection || []).forEach((t) => {
        ...
        if (inT) t.viewings.forEach((v) => { if (v.rating) rs.push(v.rating); });
      });
      const avg = rs.length ? (rs.reduce((s, r) => s + r, 0) / rs.length).toFixed(1) : null;
      lines.push(avg
        ? `${best.role} ${best.name} - you average ${avg}/10 on their work.`
        : `${best.role} ${best.name}, who you've watched before.`);
    }
  }
  // genre lane evidence, named only when it actually moved the number
  const w = getWeights(taste);
  const lanes = (item.genreIds || [])
    .map((g) => ({ name: MOVIE_GENRES[g] || TV_GENRES[g], wt: w[g] || 0 }))
    .filter((x) => x.name);
  ...
  if (bits.length) {
    const wNote = crowd && crowd.weight >= 0.3 ? "and the crowd usually lines up with you, so it counts"
      : crowd && crowd.weight <= 0.15 ? "but you often disagree with the crowd, so it barely counts"
      : "and it counts for a modest share";
...
function Modal({ onClose, children, wide }) {
  ...
  const onTS = (e) => { if (e.currentTarget.scrollTop <= 0) dragStartY.current = e.touches[0].clientY; };
  const onTM = (e) => {
    if (dragStartY.current == null) return;
    if (e.touches[0].clientY - dragStartY.current > 90) { dragStartY.current = null; onClose(); }
  };
  const onTE = () => { dragStartY.current = null; };
  return (
    <div className="modal-veil" onClick={onClose}>
      <div className={"modal-card" + (wide ? " modal-wide" : "")} onClick={(e) => e.stopPropagation()} onTouchStart={onTS} onTouchMove={onTM} onTouchEnd={onTE}>
...
function LogForm({ initial, onSave, onCancel, saveLabel, mediaType, tmdb, item }) {
  ...
  useEffect(() => {
    ...
    let active = true;
    tmdb.details("tv", item.tmdbId)
      .then((d) => { if (active) setSeasons((d.seasons || []).filter((s) => s.episode_count > 0)); })
      .catch(() => {});
    return () => { active = false; };
    ...
    <div className="log-form">
      ...
      {isTv && seasons && seasons.length > 0 && (
        <>
          ...
          <select className="field-input" value={season == null ? "" : String(season)} onChange={(e) => setSeason(e.target.value === "" ? null : Number(e.target.value))}>
      ...
      <div className="form-actions">
        ...
        <button
          ...
          onClick={() => onSave({ id: initial?.id || uid(), date: effectiveDate, undated: dateMode === "anytime", location, rating, notes, season: isTv ? season : null, loggedAt: Date.now() })}
        >
          {saveLabel || "Save"}
        </button>
      </div>
          ...
          <div className="td-back">
            {backdrop ? (
              <div className="td-hero" onClick={() => setShowPoster(true)}>
                ...
                <div className="td-hero-overlay">
                  ...
                  <div className="td-hero-genres">{ticket.mediaType === "tv" ? "TV SHOW" : "MOVIE"}</div>
                </div>
              </div>
            ) : null}
            <div className="td-back-header" style={backdrop ? { marginTop: 12 } : {}}>
              ...
              <div className="td-back-meta">
                ...
                {!backdrop && (
                  <div className="td-back-genres">{ticket.mediaType === "tv" ? "TV SHOW" : "MOVIE"}</div>
                )}
                {ticket.viewings.length > 1 && (
            ...
            <div className="viewing-list">
              {(() => {
                const sortedViewings = ticket.viewings
                  .slice()
                  .sort((a, b) => ((a.date || "") < (b.date || "") ? 1 : -1));
                const renderViewing = (v) => (
                ...
                const keys = [...groups.keys()].sort((a, b) =>
                  a === "whole" ? 1 : b === "whole" ? -1 : a - b
                );
                return keys.map((k) => (
                  <div className="season-group" key={String(k)}>
                    <div className="season-group-label">{k === "whole" ? "Whole show" : `Season ${k}`}</div>
              ...
              {logging && (
                <div className="viewing-row viewing-row-new">
                  ...
                  <LogForm mediaType={ticket.mediaType} tmdb={tmdb} item={ticket} saveLabel="Add to ticket" onSave={handleSaveViewing} onCancel={() => setLogging(false)} />
                </div>
              )}
            </div>
    ...
    <div className="view">
      ...
      {loggingWl && (
        <Modal onClose={() => setLoggingWl(null)}>
          ...
          <LogForm mediaType={loggingWl.mediaType} tmdb={tmdb} item={loggingWl} saveLabel="Add to collection" onCancel={() => setLoggingWl(null)} onSave={(entry) => { onLogNew(loggingWl, entry); setLoggingWl(null); }} />
        </Modal>
      )}
...
function SwipeCard({ item, matchPct, matchConf, taste, people, crowd, collection, tmdb, onSkip, onWant, onRate, onTapInfo }) {
  ...
  function up() {
    if (choice) return;
    if (drag.x > 100) { askChoice(); return; }       // right = seen it / want to watch
    else if (drag.x < -100) { fly("left", onSkip); return; }  // left = don't care
    setDrag({ x: 0, active: false });
  }
  ...
  return (
    ...
    >
      ...
      {showRingInfo && (
        <div className="ring-info-overlay" onTouchStart={(e) => e.stopPropagation()} onClick={(e) => { e.stopPropagation(); setShowRingInfo(false); }}>
          <div className="ring-info-card" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="ring-info-lines">
              {explainMatch(item, taste, people, crowd, collection).lines.map((l, i) => (
                <div className="ring-info-line" key={i}>{l}</div>
              ))}
            </div>
      ...
      >
        {item.posterPath ? (
          <>
            <img src={tmdbImg(item.posterPath, "w500")} alt="" className="swipe-poster-blur" draggable={false} />
      ...
      <div className="swipe-meta">
        ...
        <WhyWatch item={item} matchPct={matchPct} />
      </div>
      ...
      {choice && (
        <div className="choice-overlay" onMouseDown={(e) => e.stopPropagation()} onTouchStart={(e) => e.stopPropagation()}>
          {choice === "choose" ? (
            <>
              <div className="choice-title">{item.title}</div>
              <button className="choice-btn choice-btn-want" onClick={() => fly("right", onWant)}>WANT TO WATCH</button>
              <button className="choice-btn choice-btn-seen" onClick={() => setChoice("rate")}>I'VE SEEN IT</button>
              <button className="choice-dismiss" onClick={() => setChoice(null)}>not now</button>
            </>
          ) : (
            <>
              <div className="choice-title">Rate it</div>
              <div className="choice-stars"><Stars value={rateVal} onChange={setRateVal} size={30} /></div>
              <button
                className="choice-btn choice-btn-want"
                disabled={!rateVal}
                style={!rateVal ? { opacity: 0.45 } : undefined}
                onClick={() => rateVal && fly("up", () => onRate(item, { id: uid(), date: todayISO(), undated: false, location: "", rating: rateVal, notes: "", loggedAt: Date.now() }))}
              >LOG IT</button>
              <button className="choice-dismiss" onClick={() => setChoice("choose")}>back</button>
            </>
          )}
        </div>
      )}
...
function DiscoverView({ tmdb, feedback, setFeedback, taste, people, settings, collection, watchlist, onAddToWatchlist, onLogNew, active }) {
  ...
  useEffect(() => {
    ...
    return () => { if (washTimer.current) clearTimeout(washTimer.current); };
  }, [grad]);
  ...
  const loadPool = useCallback(async (includeSkipped = false) => {
    ...
    try {
      ...
      const calls = [
        ...
        tmdb.discoverMovie({ sort_by: "popularity.desc", page: pageNum, with_original_language: "en", "vote_count.gte": 1000, "primary_release_date.gte": `${yr - 3}-01-01` }),
        tmdb.discoverMovie({ sort_by: "popularity.desc", page: pageNum + 1, with_original_language: "en", "vote_count.gte": 1000, "primary_release_date.gte": `${yr - 3}-01-01` })
      ];
      if (topGenres) {
      ...
      setPool((prev) => { const live = prev.filter((p) => !skipSet.has(p.tmdbId + p.mediaType) && !ownedSet.has(p.tmdbId + p.mediaType)); return [...live, ...woven]; });
    } catch (e) {
      setError(e.message);
    }
    setLoading(false);
  ...
  return (
    <div className="suggest-row" onClick={() => setExpanded((x) => !x)}>
      ...
      <div className="suggest-info">
        ...
        {taste && !item.aiReason && <WhyWatch item={item} matchPct={matchPct} />}
        {expanded && (
      ...
      {logging && (
        <Modal onClose={() => setLogging(false)}>
          <h3 className="modal-title">{item.title}</h3>
          <LogForm mediaType={item.mediaType} tmdb={tmdb} item={item} saveLabel="Add to collection" onCancel={() => setLogging(false)} onSave={(entry) => { onSeen(item, entry); setLogging(false); }} />
        </Modal>
      )}
    </div>
  );
      ...
      <div className="view">
        {detail && (
          <DetailModal
            ...
            settings={settings || {}}
            onClose={() => setDetail(null)}
            onAddToWatchlist={onAddToWatchlist ? (it) => { onAddToWatchlist(it); } : null}
        ...
        {!filmoLoading && filmo && filmo.length > 0 && (
          <div className="suggest-list">
            {filmo.map((item) => (
              <button className="suggest-row suggest-row-btn" key={item.tmdbId + item.mediaType} onClick={() => setDetail(item)} aria-label={`Details for ${item.title}`}>
                {item.posterPath ? (
                  <img src={tmdbImg(item.posterPath, "w154")} alt="" className="suggest-thumb" />
                ) : (
                  <div className="suggest-thumb suggest-thumb-fallback">{item.mediaType === "tv" ? <Tv size={18} /> : <Film size={18} />}</div>
  ...
  return (
    <div className="coming-row" onClick={() => setExpanded((x) => !x)}>
      ...
      <div className="coming-info">
        ...
        {expanded && (
          <div className="suggest-links" onClick={(e) => e.stopPropagation()}>
            <a className="link-pill" href={`https://www.youtube.com/results?search_query=${encodeURIComponent(item.title + " trailer")}`} target="_blank" rel="noreferrer">Trailer</a>
            <a className="link-pill" href={buildRedditLink(item.title, item.year)} target="_blank" rel="noreferrer">Reddit</a>
          </div>
        )}
      </div>
    ...
    <div className="view">
      ...
      {logging && (
        <Modal onClose={() => setLogging(null)}>
          ...
          <LogForm mediaType={logging.mediaType} tmdb={tmdb} item={logging} saveLabel="Add to collection" onCancel={() => setLogging(null)} onSave={(entry) => { onLogNew(logging, entry); setLogging(null); }} />
        </Modal>
      )}
  ...
  async function runSearch(e) {
    ...
    try {
      if (isAsk) {
        ...
        ai.forEach((it) => { it._pct = matchMeta(it, taste, people, crowd).pct; });
    ...
    <div className={"app" + (tab === "discover" ? " wash-on" : "")}>
      ...
      {showFavorites && (
        <Modal onClose={() => setShowFavorites(false)} wide>
          ...
          <FavoritesView collection={collection} people={people} taste={taste} crowd={crowd} tmdb={tmdb} settings={settings} onUpdateTicket={updateTicket} onAddToWatchlist={addToWatchlist} onLogNew={logNew} />
        </Modal>
      )}
    </div>
  );
...
.top-five-label { font-weight: 700; font-size: 14px; color: #3a2a20; }
.top-five-sub { font-size: 11.5px; color: #8a6f57; margin: 2px 0 10px; }
